        ...
        @foreach ($company->adverts as $advert)
            @foreach ($advert->applications as $application)
                <div class="flex flex-row flex-wrap items-center text-center even:bg-gray-200 py-2">
                    ...
                    <p class="w-5/12">{{ $applicant->email }}</p>
                    @can('view', $application)
                        <x-secondary-link href="{{ route('application.show', $application) }}" class="w-1/12">
                    ...
                    @switch($application->status)
                        ...
                        @case('denied')
                            <p
                                class="w-1/12 text-xs text-yellow-400 bg-red-900 border border-yellow-400 rounded-full py-1 px-2">
